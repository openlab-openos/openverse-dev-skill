...
description: Openverse Kit plugin architecture, all-in-one RPC/LiteSVM plugins, signer plugins, custom client composition, and plugin ordering rules.
---

# Openverse Kit Plugins & Client Composition
...
const client = createClient()
  ...
  .use(solanaRpc({ rpcUrl: 'https://api.mainnet.openverse.network' }));
...
// Devnet — defaults to https://api.devnet.openverse.network, includes airdrop
...
const client = createClient()
  ...
  .use(solanaRpc({ rpcUrl: 'https://api.mainnet.openverse.network' })); // bundles tx planning + sending
...
| `airdropSigner(amount)` / `airdropPayer(amount)` / `airdropIdentity(amount)` | Airdrop BTG to an already-installed signer. Use with all-in-one `solanaLocalRpc` / `solanaDevnetRpc` / `litesvm` when the RPC plugin needs a payer first. |
...
const client = await createClient()
  .use(rpc('https://api.devnet.openverse.network'))    // adds client.rpc + client.rpcSubscriptions
