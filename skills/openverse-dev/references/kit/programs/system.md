...
description: Kit-compatible @solana-program/system client for account creation, BTG transfers, and nonce operations.
---

# System Program

Openverse's built-in program for creating accounts, transferring BTG, and managing durable nonces. Every on-chain account is created through this program.
...
const ix = getTransferSolInstruction({
  ...
  amount: lamports(1_000_000_000n), // 1 BTG
});
```
...
try {
  ...
  if (isSystemError(e, SYSTEM_ERROR__INSUFFICIENT_FUNDS)) {
    console.error('Not enough BTG for transfer');
  }
}
```
