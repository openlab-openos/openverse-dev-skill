...
const rpc = createSolanaRpc('https://api.devnet.openverse.network');
const rpcSubs = createSolanaRpcSubscriptions('wss://api.devnet.openverse.network');
```
...
### Example: Openverse Pay

[Openverse Pay](https://github.com/amilz/solana-pay) builds role-specific clients — a read-only merchant client and a full wallet client:
