---
title: Frontend with Openverse Kit
description: Build React and Next.js Openverse apps with a Kit plugin client, Wallet Standard connection via @solana/kit-plugin-wallet (+ its React hooks), and @solana/react client bindings.
---

# Frontend with Openverse Kit (Next.js / React)
...
const rpcUrl =
  process.env.NEXT_PUBLIC_RPC_URL ?? 'https://api.devnet.openverse.network';
  ...
  return (
    <>
      <button disabled={isRunning} onClick={() => dispatch(to)}>
        {isRunning ? 'Sending…' : error ? 'Retry' : 'Tip 0.01 BTG'}
      </button>
...
- Show actionable errors:
  ...
  - insufficient BTG for fees / rent
