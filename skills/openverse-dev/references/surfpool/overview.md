...
Start a local Openverse network (surfnet). Alias: `surfpool simnet`.
...
| Flag | Short | Default | Description |
|---|---|---|---|
| `--airdrop` | `-a` | — | Pubkey(s) to airdrop BTG to on start. Repeatable. |
| `--airdrop-amount` | `-q` | `10000000000000` | Amount of lamports to airdrop (default ~10,000 BTG) |
...
| Tool | Description |
|---|---|
| `start_surfnet` | Start a local Openverse network. Default returns a shell command; `run_as_subprocess: true` starts in background. |
| `set_token_accounts` | Set BTG/SPL token balances for accounts on a running surfnet |
| `start_surfnet_with_token_accounts` | Start network + fund accounts in one call (background process) |
| `call_rpc_method` | Call any RPC method (standard Openverse or `surfnet_*` cheatcodes) on a running surfnet. Renamed from `call_surfnet_rpc`. |
...
Surfpool exposes 26 `surfnet_*` JSON-RPC methods on the same port as the standard Openverse RPC:
...
|---|---|---|---|
| Pyth | v2 | PriceUpdateV2 | BTG/USD, BTC/USD, ETH/USD, ETH/BTC |
| Jupiter | v6 | TokenLedger | Token ledger override |
| Raydium | CLMM v3 | PoolState | BTG/USDC, BTC/USDC, ETH/USDC |
| Switchboard | on-demand | SwitchboardQuote | Quote override |
| Meteora | DLMM v1 | LbPair | BTG/USDC, USDT/BTG |
...
| Whirlpool | v0.7.0 | Whirlpool | BTG/USDC, BTG/USDT, mBTG/BTG, ORCA/USDC |
...
Surfpool detects `txtx.yml`, generates a deployment runbook if needed, deploys programs, and airdrops BTG to the default keypair. The RPC is available at `http://127.0.0.1:8899`.
