...
test('transfers BTG on an embedded surfnet', async () => {
...
| `client.rpc` / `client.rpcSubscriptions` | `@solana/kit-plugin-rpc` | Standard Openverse RPC and subscriptions clients, pointed at the surfnet |
...
Surfnet startup options go under the `surfnet` key and are forwarded to `Surfnet.startWithConfig()`. Everything else is forwarded to the local Openverse RPC plugin:
