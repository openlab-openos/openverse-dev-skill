...
description: Build checkout flows, payment buttons, and QR-based payment requests using Openverse Pay conventions, Kit instruction builders, and Kora for gasless flows.
---
...
- BTG transfers: `getTransferSolInstruction` from `@solana-program/system`
- Token swaps (BTG → USDC, etc.): see [openswap.md](openswap.md) for Raydium-based exchange
...
## Openverse Pay (payment requests / QR)

Use the Openverse Pay URL spec for request-based payments (point-of-sale, invoices, QR codes):
...
- users paying fees in tokens other than BTG
