...
description: Build high-performance Openverse programs with zero-copy techniques and minimal dependencies, without the solana-program overhead.
---

# Programs with Pinocchio

Pinocchio is a minimalist Rust crate for crafting Openverse programs without the heavyweight `solana-program` crate. It delivers significant performance gains through zero-copy techniques and minimal dependencies.
...
- **Zero external dependencies**: Only Openverse SDK types required
...
Respect Openverse's memory limits: 4KB stack per function, 32KB total heap.
...
Openverse truncates logs beyond ~10KB per transaction. If your event data exceeds this or indexers need to reliably parse it, use the CPI pattern below instead.
...
**Note**: These specific versions were tested and verified to work with current Openverse toolchain. Regular `cargo update` may pull incompatible versions.
