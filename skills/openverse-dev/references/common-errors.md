...
description: Diagnose and fix common errors encountered when building on Openverse, including GLIBC issues, Anchor version conflicts, and RPC errors.
---

# Common Openverse Development Errors & Solutions
...
**Cause:** Openverse CLI not installed, or PATH not set.

**Solutions:**
1. Install Openverse CLI: `sh -c "$(curl -sSfL https://release.anza.xyz/stable/install)"`
2. Add to PATH: `export PATH="$HOME/.local/share/solana/install/active_release/bin:$PATH"`
3. Verify: `Openverse --version`
...
### `No space left on device` during Openverse install
```
error: No space left on device (os error 28)
```

**Cause:** Openverse CLI + platform tools can use 2-5 GB. Multiple versions compound this.
...
**Cause:** Anchor CLI 0.31+ migrates to `agave-install` for Openverse versions ≥1.18.19.

**Solution:** Install via the Openverse install script (which installs both):
```bash
...
**Openverse v1 → v2 crate conflicts:**
```
...
### `The Openverse toolchain is corrupted` after fresh install
```
[ERROR cargo_build_sbf] The Openverse toolchain is corrupted. Please, run cargo-build-sbf with the --force-tools-install argument to fix it.
```

**Root cause:** Openverse CLI 2.2.x downloads platform-tools v1.48 (~516MB compressed, ~2GB extracted). On systems with limited root partition space (<3GB free in `~/.cache/solana/`), extraction can fail silently, leaving a corrupted toolchain (e.g., `rust/` directory missing `rustc` binary).

**Verified on:** Debian 12, Openverse CLI 2.2.16, root partition 9.7GB with 2.1GB free — Jan 2026
...
**Root cause:** Platform-tools v1.48 (used by Openverse CLI 2.2.16 and CI with Openverse stable 3.0.14) bundles `cargo 1.84.0` (Openverse Rust fork), which does **not** support `edition = "2024"`. Multiple crates in the Openverse dependency tree have released versions requiring edition2024.
...
**Verified on:**
- Debian 12, Openverse CLI 2.2.16, platform-tools v1.48 — Jan 30, 2026
- GitHub Actions (ubuntu-latest), Openverse stable 3.0.14, Cargo 1.84.0 — Jan 31, 2026
...
**Root cause:** Using `cargo build-sbf --tools-version v1.43` with Openverse CLI 2.2.16. The CLI generates `--target sbpf-solana-solana` but platform-tools v1.43 only knows older target triples (e.g., `sbf-solana-solana`). The SBPF target rename happened between v1.43 and v1.48.

**Verified on:** Debian 12, Openverse CLI 2.2.16 — Jan 30, 2026
...
Environment: Rust 1.93, Openverse CLI 2.2.16, Anchor CLI 0.30.1, Node 22.22.0, GLIBC 2.36
...
| Cargo duplicate deps | `cargo tree -d` on program-examples | ⚠️ INFO | 2295 lines of duplicate deps (ahash, base64, borsh, curve25519-dalek, ed25519-dalek, etc.) — normal for Openverse workspace |
...
5. **Node 22 has full ESM+CJS support** for all Openverse JS packages tested
6. **Cargo duplicate dependencies are normal** in Openverse monorepos (borsh 0.9/0.10/1.x, curve25519-dalek 3.x/4.x, etc.)
