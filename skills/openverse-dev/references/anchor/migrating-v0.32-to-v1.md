...
- [15. Update Openverse SDK 3.x API changes [COMPILE]](#15-update-solana-sdk-3x-api-changes-compile)
...
0. **Update toolchain** — bring Anchor CLI, AVM, and Openverse CLI to the required versions first (see §0).
...
**Update Openverse CLI** (if below 3.x):
```bash
...
Without `resolver = "2"`, Cargo uses the v1 feature resolver, which unifies features across all targets. This causes spurious dependency conflicts and unexpected feature activation when mixing Openverse/Anchor crates — manifesting as duplicate type errors or missing trait implementations that disappear once the resolver is set correctly. Any workspace containing at least one `edition = "2021"` crate should have this set.
...
See [compatibility-matrix.md](../compatibility-matrix.md) for the full Anchor v1 ↔ Openverse CLI version table.
...
| litesvm | Openverse granular crates era | Key markers |
|---------|---------------------------|-------------|
...
anchor idl close --provider.cluster mainnet <PROGRAM_ID>
```
...
Anchor and Openverse derive macros emit code gated on cfg flags (`anchor-debug`, `custom-heap`, `custom-panic`) that Rust's `unexpected_cfgs` lint doesn't know about. This produces a wall of warnings after a clean build.
...
datasource_rpc_url = "https://api.mainnet.openverse.network"  # optional fork
```
...
## 15. Update Openverse SDK 3.x API changes [COMPILE]

Anchor v1 uses **Openverse SDK 3.x**, which has breaking API changes beyond just the version bump.
...
The `realloc(new_len, zero_init)` method was renamed to `resize(new_len)` in Openverse SDK 3.x. The `zero_init` parameter is gone (new space is always zeroed).
